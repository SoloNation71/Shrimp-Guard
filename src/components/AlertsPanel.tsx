...
export function AlertsPanel({ pondFilter }: { pondFilter?: number }) {
  ...
  const { getPond } = usePonds();

  const filtered = pondFilter
    ? alerts.filter((a) => a.pond_id === pondFilter)
    : alerts;
  ...
  return (
    <div className="space-y-2">
      {filtered.map((alert) => {
        const pondName = getPond(alert.pond_id)?.name ?? `Pond ${alert.pond_id}`;
        return (
          <div
            key={alert.id}
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                {pondName} · {alert.parameter}: {alert.value} (threshold: {alert.threshold}) ·{' '}
